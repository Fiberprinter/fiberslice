...
Clustering models"
            ...
            let mut root = polygon_faces.clone().into_iter().fold(
                Mask::create_root(min.xzy(), max.xzy()),
                |mut root, face| {
                    root.push_face(face);
    ...
    // i love you
    pub fn insert(&mut self, model_handle: LoadResult) -> Result<Arc<Mask>, Error> {
...
impl InteractiveModel for Mask {
